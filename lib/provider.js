...
function systemPrompt(vision) {
  const sees = vision
    ? 'You receive a screenshot of the page, a structured text summary of it, and the person\'s spoken question.'
...
Actions:
- You may propose ONE action only when the person clearly asks for it (for example "click the login button", "put my email in the box", "take me to the search field").
- Use the index from the INTERACTIVE ELEMENTS list.
- If nothing matches, set action to null and explain what you see instead.

Respond ONLY with strict JSON, no markdown fences:
{"answer": "<what to say out loud>", "lang": "<BCP-47 tag of the answer, like en or es>", "action": null | {"type": "click" | "focus" | "fill", "index": <number>, "value": "<text to type, only for fill>"}}`;
}
