...
(() => {
  ...
  function scrollInfo() {
    ...
    const max = Math.max(1, doc.scrollHeight);
    const top = Math.round((window.scrollY / max) * 100);
    const bottom = Math.min(100, Math.round(((window.scrollY + window.innerHeight) / max) * 100));
    const screens = Math.max(1, Math.round(doc.scrollHeight / window.innerHeight));
    if (screens <= 1) return 'SCROLL POSITION: the whole page fits on one screen.';
    return `SCROLL POSITION: the user is currently viewing from ${top}% to ${bottom}% of a page about ${screens} screens tall. The PAGE TEXT and the screenshot may cover the whole page, beyond what the user currently sees.`;
  }
